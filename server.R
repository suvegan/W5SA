...
shinyServer(function(input, output){
  ...
  if(input$city_dropdown == 'All') {
    ...
    city_weather_bike_df$FORECASTDATETIME <- as.POSIXlt(city_weather_bike_df$FORECASTDATETIME,origin ="1970-01-01")
    glimpse(city_weather_bike_df)
    ...
    filteredData$FORECASTDATETIME <- as.POSIXlt(filteredData$FORECASTDATETIME,origin ="1970-01-01")
    print(head(filteredData))
    
    filteredData2<- city_weather_bike_df %>%
      filter(CITY_ASCII == input$city_dropdown)
    
    filteredData2$FORECASTDATETIME <- as.POSIXct(filteredData2$FORECASTDATETIME,origin ="1970-01-01")
    
    glimpse(filteredData2)
    
    output$temp_line <- renderPlot({ggplot(data=filteredData2,aes(x =FORECASTDATETIME,y=as.integer(TEMPERATURE)))+
        
        geom_line(linetype = "solid",color = "yellow", size = 1)+scale_x_datetime(breaks = date_breaks("1 day"),date_labels = "%d/%m/%y")+scale_y_continuous(limits = c(0, 22.5), n.breaks =3)+
        geom_point()+geom_text(label = filteredData2$TEMPERATURE,check_overlap = TRUE,
                               na.rm = FALSE,
                               position = position_dodge(width = 1),vjust=-0.5,hjust=-0.4)+
        labs(y="TEMPERATURE(C)",x= "Time (3 hours ahead)" ,title ="Temperature Chart")})
    
    
    output$bike_line <- renderPlot({ggplot(data= filteredData2,aes(x =FORECASTDATETIME,y=BIKE_PREDICTION))+
        ...
        geom_point()+geom_text(label = filteredData2$BIKE_PREDICTION,check_overlap = TRUE,
                               na.rm = FALSE,
                               position = position_dodge(width = 1),vjust=-0.5,hjust=-0.4)+
    ...
    output$TIME<-renderPrint({return(paste0("TIME ", as_datetime(input$plot_click$x)))})
    
    
    
    output$bike_date_output <- renderText({return(paste0("BikeCountPred ",as.integer(input$plot_click$y)))})
